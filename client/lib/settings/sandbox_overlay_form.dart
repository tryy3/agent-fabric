...
class SandboxOverlayForm extends StatefulWidget {
  const SandboxOverlayForm({
    super.key,
  ...
  final Future<void> Function(Map<String, dynamic> sandbox) onSave;
  ...
  @override
  State<SandboxOverlayForm> createState() => SandboxOverlayFormState();
}
...
class SandboxOverlayFormState extends State<SandboxOverlayForm> {
  ...
  void _apply(Map<String, dynamic> sandbox) {
    _kind = sandbox['kind'] as String? ?? (widget.embedded ? '' : 'docker');
  ...
  Map<String, dynamic> _patch() {
    ...
    return {
      ...
      if (_workspaceRoot.text.trim().isNotEmpty)
        'workspaceRoot': _workspaceRoot.text.trim(),
      if (_image.text.trim().isNotEmpty) 'image': _image.text.trim(),
      if (_containerName.text.trim().isNotEmpty)
        'containerName': _containerName.text.trim(),
      'idleTTLSeconds': ?ttl,
      if (_volumes.isNotEmpty || _removedVolumeIDs.isNotEmpty)
        'volumes': [
          for (final volume in _volumes) volume.toJson(),
          for (final id in _removedVolumeIDs) {'id': id, 'enabled': false},
        ],
      if (_extraPaths.isNotEmpty || _removedPathIDs.isNotEmpty)
        'extraPaths': [
          for (final extra in _extraPaths) extra.toJson(),
          for (final id in _removedPathIDs) {'id': id, 'enabled': false},
        ],
    ...
    final body = Column(
      ...
      children: [
        ...
        DropdownButtonFormField<String>(
          ...
          initialValue: _kind.isEmpty && !widget.embedded ? 'docker' : _kind,
          items: [
            if (widget.embedded)
              const DropdownMenuItem(value: '', child: Text('inherit')),
        ...
        TextField(
          ...
          decoration: const InputDecoration(
            ...
            helperText: 'Blank inherits the parent overlay',
          ),
        ),
        const SizedBox(height: 16),
        TextField(
