...
func TestResolveUnderRootAcceptsDotRelative(t *testing.T) {
	root := t.TempDir()
	got, err := sandbox.ResolveUnderRoot(root, "./a.txt")
	if err != nil {
		t.Fatal(err)
	}
...
func TestResolveUnderRootAcceptsAbsoluteUnderRoot(t *testing.T) {
	root := t.TempDir()
	absRoot, err := filepath.Abs(root)
	if err != nil {
		t.Fatal(err)
	}
	got, err := sandbox.ResolveUnderRoot(absRoot, filepath.Join(absRoot, "root.json"))
	if err != nil {
		t.Fatal(err)
	}
	want := filepath.Join(absRoot, "root.json")
	if got != want {
		t.Fatalf("got %q want %q", got, want)
	}
}
...
func TestResolveUnderRootAcceptsRootItself(t *testing.T) {
	...
	got, err := sandbox.ResolveUnderRoot(absRoot, absRoot)
	if err != nil {
		t.Fatal(err)
	}
	if got != absRoot {
...
func TestResolveUnderRootRejectsAbsoluteOutsideRoot(t *testing.T) {
	root := t.TempDir()
	_, err := sandbox.ResolveUnderRoot(root, "/not-workdir/x")
